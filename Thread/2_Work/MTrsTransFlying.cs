...
{
    public class MTrsTransFlying : MSystem
    {
        #region Variable
        ...
        const int STEP_MOVE_START_X = 2000;
        const int STEP_MOVE_END_X = 4000;
        ...
        public void dorunStep()
        {
            ...
            switch (m_iCurrentStep)
            {
                ...
                case STEP_MOVE_XY + 100:
                    ...
                    break;
                case STEP_MOVE_START_X:
                    MovePosition(Axis.AXIS_X, PosEndX());
                    SetStep(STEP_MOVE_START_X + 100);

                    break;
                case STEP_MOVE_START_X + 100:
                    if (!IsMoveComplete(Axis.AXIS_X, CalculatorPosX(InforTeaching.Instance.TrayColumns - 1)))
                    {
                        SetStep(STEP_MOVE_START_X);
                        break;
                    }
                ...
                case STEP_MOVE_END_X:
                    MovePosition(Axis.AXIS_X, PosStartX());
                    SetStep(STEP_MOVE_END_X + 100);

                    break;
                case STEP_MOVE_END_X + 100:
                    if (!IsMoveComplete(Axis.AXIS_X, CalculatorPosX(0)))
                    {
                        SetStep(STEP_MOVE_END_X);
                        break;
                    }
                    index_Y++;
                    SetStep(STEP_MOVE_Y);
        ...
        bool CheckMoveComplete(double dCurPos, double tar)
        {
            double tar_pos = tar;
